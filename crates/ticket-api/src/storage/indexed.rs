...
/// Metadata stored per-ticket in the redb index.
/// Does not hold full content — that lives in `ticket.toml` on disk.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct IndexedTicket {
    pub id: Uuid,
    /// Absolute path to the ticket folder (contains `ticket.toml`).
    pub path: PathBuf,
    pub type_id: String,
    ...
    /// Soft-delete flag. Deleted tickets are kept in the index for audit.
    pub deleted: bool,
}
