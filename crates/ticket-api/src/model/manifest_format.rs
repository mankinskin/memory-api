...
/// Canonical ordering for top-level fields in `ticket.toml`.
///
/// Fields listed here appear at the top of every ticket file, in this exact
/// order.  Any additional fields not covered by this list are written in
/// **alphabetical order** after the priority fields.
///
/// DX rationale:
/// - `id` + `created_at`         — immutable identity; always first for quick scanning.
/// - `title` + `state`           — the two things a developer reads at a glance.
/// - `acceptance_criteria`       — the "definition of done"; belongs near the top.
...
/// Serialize a `TicketManifest` to a canonical TOML string.
///
...
///
/// The output is always valid TOML and round-trips through `toml::from_str`
/// without data loss.
pub fn format_manifest_toml(manifest: &TicketManifest) -> String {
    ...
    // Skip the first two slots ("id", "created_at") — they are struct fields,
    // not stored in `extra`.
    ...
    // BTreeMap already iterates in sorted key order, so we only need to skip
    // the fields already emitted above.
...
/// Returns `true` when every field in `toml_text` is already in canonical
/// order.
///
/// This is a pure structural check (no deserialization) designed for the
/// health-check hot path.
...
pub fn canonical_order_for_keys(keys: &[String]) -> Vec<String> {
    ...
    // Priority fields first, in canonical order (only those actually present).
    ...
    // Remaining fields: alphabetical sort, mirroring BTreeMap iteration order.
...
///
/// Handles the subset of TOML used in ticket.toml files: flat `key = value`
/// pairs, with optional multi-line basic strings (`"""..."""`).  Does not
/// parse nested tables or dotted keys — ticket manifests never use them.
fn extract_key_order(toml_text: &str) -> Vec<String> {
    ...
    for line in toml_text.lines() {
        ...
        // Inside a multi-line string: scan for the closing `"""`.
        if in_multiline {
            if trimmed.contains("\"\"\"") {
                in_multiline = false;
            }
        ...
        // Match `key = value` (TOML standard spacing used in our output).
        if let Some(eq_pos) = trimmed.find(" = ") {
            let key = trimmed[..eq_pos].trim().to_string();
            let value_part = trimmed[eq_pos + 3..].trim();

            // Multi-line basic string: value starts with `"""` and the closing
            // `"""` is NOT on the same opening line.
...
fn write_toml_kv(out: &mut String, key: &str, value: &Value) {
    match value {
        ...
        Value::Null => {
            // TOML has no null; emit an empty string as a stable fallback.
            writeln!(out, "{key} = \"\"").unwrap();
        }
    }
}

/// Produce an inline TOML representation of a JSON value (no newlines).
...
///
/// Handles all TOML escape sequences, including `\n` for embedded newlines
/// (ticket fields may contain newlines, e.g. multi-line acceptance criteria).
...
mod tests {
    ...
    fn make_manifest(extra: &[(&str, &str)]) -> TicketManifest {
        let mut m = TicketManifest::new(Uuid::new_v4(), Utc::now());
        for (k, v) in extra {
    ...
    /// Round-trip a manifest through the formatter and TOML parser.
    ///
    /// Returns the reparsed `TicketManifest`.  Panics if the formatter
    /// produces invalid TOML.
    fn roundtrip(m: &TicketManifest) -> TicketManifest {
        let toml = format_manifest_toml(m);
        toml::from_str(&toml).expect("formatted TOML should be valid")
    }

    // ── field preservation (no drops) ────────────────────────────────────────

    #[test]
    fn no_fields_dropped_string_fields() {
        // Covers priority fields + many extra fields.
        let m = make_manifest(&[
        ...
        for key in m.extra.keys() {
            assert!(
                parsed.extra.contains_key(key),
                "field '{key}' was dropped by the formatter"
            );
            assert_eq!(
                parsed.extra[key], m.extra[key],
        ...
        assert_eq!(
            parsed.extra.len(),
            m.extra.len(),
            "extra field count changed: some field was silently added or removed"
        );
    }
    ...
    fn no_fields_dropped_large_field_set() {
        // Stress-test: 30 fields including all canonical priority fields.
        let mut m = TicketManifest::new(Uuid::new_v4(), Utc::now());
        ...
        assert_eq!(
            parsed.extra.len(),
            m.extra.len(),
            "field count changed: expected {}, got {}",
            m.extra.len(),
            parsed.extra.len()
        );
        for (key, _) in &fields {
    ...
    // ── no content modified (struct-level equality) ───────────────────────────

    #[test]
    fn manifest_partialeq_holds_after_roundtrip() {
        // PartialEq covers id + created_at + all extra fields.
        // If ANY field is dropped or ANY value changed, this fails.
        let mut m = TicketManifest::new(Uuid::new_v4(), Utc::now());
        ...
        assert_eq!(
            parsed, m,
            "roundtripped manifest does not equal original — field dropped or value changed"
        );
    }

    #[test]
    fn created_at_is_preserved_exactly() {
        // DateTime<Utc> must survive format → parse without precision loss.
        let fixed = chrono::DateTime::parse_from_rfc3339("2026-04-08T14:20:50.462259100+00:00")
            .unwrap()
            .with_timezone(&Utc);
        let m = TicketManifest::new(Uuid::new_v4(), fixed);
        let parsed = roundtrip(&m);
        assert_eq!(
            parsed.created_at, m.created_at,
            "created_at was altered during format/parse"
        );
    }
    ...
    fn roundtrip_string_with_mixed_special_chars() {
        // Backslash + quotes + newline in the same string.
        let value = "path: \"C:\\tmp\"\nnext line";
    ...
    fn formatting_is_idempotent() {
        // format → parse → format must produce the same string.
        let mut m = TicketManifest::new(Uuid::new_v4(), Utc::now());
        ...
        let reparsed: TicketManifest = toml::from_str(&first_format).unwrap();
        let second_format = format_manifest_toml(&reparsed);

        assert_eq!(
            first_format, second_format,
            "formatting is not idempotent: second pass differed from first"
        );
    }

    #[test]
    fn formatting_is_idempotent_after_is_canonically_ordered_check() {
        // After one format pass, is_canonically_ordered must return true for
        // the output, AND a second pass must not change the string.
        let m = make_manifest(&[
            ("zzz_last", "z"),
            ("state", "ready"),
        ...
        assert!(
            ...
            "formatted output is not considered canonically ordered"
        );
        let reparsed: TicketManifest = toml::from_str(&formatted).unwrap();
        assert_eq!(format_manifest_toml(&reparsed), formatted);
    }

    // ── existing tests (ordering / detection) ────────────────────────────────
    ...
    fn roundtrip_boolean_and_number() {
        ...
        let toml = format_manifest_toml(&m);
        let parsed: TicketManifest = toml::from_str(&toml).unwrap();
