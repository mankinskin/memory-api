...
mod tests {
    ...
    fn test_create_escalation_writes_correct_json() {
        ...
        assert_eq!(parsed["escalation_id"].as_str().unwrap(), escalation.escalation_id);
        assert_eq!(parsed["status"].as_str().unwrap(), "open");
        assert_eq!(parsed["blocking_decision"].as_str().unwrap(), "Cannot decide between A and B");
        assert_eq!(parsed["context"].as_str().unwrap(), "Both options have trade-offs");
        assert_eq!(parsed["requested_capability"].as_str().unwrap(), "architecture decision");
    ...
    fn test_resolve_escalation_all_actions() {
        ...
        assert_eq!(resolved1.resolution.as_ref().unwrap().action, EscalationAction::Handled);
        ...
        assert_eq!(resolved2.resolution.as_ref().unwrap().action, EscalationAction::GrantedOffset);
        assert_eq!(resolved2.resolution.as_ref().unwrap().offset_grant_id, Some("grant-123".to_string()));
        ...
        assert_eq!(resolved3.resolution.as_ref().unwrap().action, EscalationAction::SpawnedSession);
        assert_eq!(resolved3.resolution.as_ref().unwrap().spawned_session_id, Some("session-new".to_string()));
        ...
        assert_eq!(resolved4.resolution.as_ref().unwrap().action, EscalationAction::EscalatedToUser);
    }
