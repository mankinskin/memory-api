...
fn test_existing_sessions_load_without_error_real_data() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✓ AC1: {} existing sessions loaded without error", sessions_loaded);
...
fn test_manifest_roundtrip_with_track_fields() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(deserialized.anchor_ticket_id, Some("ticket-abc".to_string()));
    assert_eq!(deserialized.parent_session_id, Some("parent-def".to_string()));
    assert_eq!(deserialized.spawned_session_id, Some("spawned-ghi".to_string()));
