use session_api::{SessionStoreConfig, SessionAuditSelector};
...
fn main() {
    ...
    let jsonl = std::path::Path::new("c:/Users/linus/git/graph_app/context-engine/tmp/mini.jsonl").to_path_buf();

    let plan = store.capture_copilot_transcript(&jsonl, "smoke").unwrap();
    println!("captured session_id={}", plan.record.session_id);

    let metrics_path = tmp.join("sessions").join(&plan.record.session_id).join("tool-metrics.json");
    let metrics_raw = std::fs::read_to_string(&metrics_path).unwrap();
    println!("tool-metrics.json bytes={} content={}", metrics_raw.len(), metrics_raw);

    let report = store.delegation_cost_report(SessionAuditSelector::SessionId(plan.record.session_id.clone())).unwrap();
    println!("delegation report: {}", serde_json::to_string_pretty(&report).unwrap());
