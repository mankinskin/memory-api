...
use session_mcp::run_mcp_server;
...
async fn main() {
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::from_default_env()
                .add_directive("session_mcp=info".parse().unwrap()),
        )
    ...
    eprintln!(
        "session-mcp starting (store: {}, workspace: {workspace_slug})",
        store_root.display()
    );
...
fn resolve_store_root() -> PathBuf {
    ...
    memory_api::workspace::resolve_requested_store_root(None, None, None, ".memory-api")
}
