...
use ticket_api::{
    ...
    storage::{board::BoardSnapshot, indexed::IndexedTicket},
...
impl TicketServer {
    pub(crate) async fn next_tickets_tool(
        ...
        let (items, board_value, excluded_by_board, warnings) = self.with_store(|store| {
            ...
            let tickets = filtered_tickets(store.list(None, None, None)?, filter.as_deref());
            let done_ids = done_ticket_ids(&tickets);
            let blockers = unresolved_blockers(&store.list_all_edges()?, &done_ids);
            let state_index = build_state_index(store);
            let mut candidates = candidate_tickets(&tickets, &done_ids, &blockers);
            let priority_map = read_priorities(&candidates);

            sort_candidates(&mut candidates, &state_index, &priority_map);
            let excluded_by_board = excluded_by_board(board_snap.as_ref(), &candidates);
...
fn filtered_tickets(all: Vec<IndexedTicket>, filter: Option<&str>) -> Vec<IndexedTicket> {
    match filter {
        Some(prefix) => all
            .into_iter()
            .filter(|ticket| {
...
fn done_ticket_ids(tickets: &[IndexedTicket]) -> HashSet<Uuid> {
    tickets
        .iter()
        .filter(|ticket| matches!(ticket.state.as_deref(), Some("done" | "cancelled")))
...
fn sort_candidates(
    ...
    candidates.sort_by(|left, right| {
        ...
        right_index
            .cmp(&left_index)
            .then_with(|| {
                let left_priority = priority_map.get(&left.id).map(String::as_str).unwrap_or("");
                let right_priority = priority_map.get(&right.id).map(String::as_str).unwrap_or("");
...
fn excluded_by_board(board_snap: Option<&BoardSnapshot>, candidates: &[&IndexedTicket]) -> Vec<Value> {
...
fn filter_board_candidates(candidates: &mut Vec<&IndexedTicket>, board_snap: Option<&BoardSnapshot>) {
    let Some(snapshot) = board_snap else {
        return;
    };
...
fn ranked_items(candidates: &[&IndexedTicket], priority_map: &HashMap<Uuid, String>) -> Vec<Value> {
    candidates
        .iter()
        .enumerate()
        .map(|(rank, ticket)| {
