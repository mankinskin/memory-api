...
use self::batch_dispatch_impl::batch_dispatch;
use self::batch_undo_impl::{BatchUndoOp, apply_batch_undo, batch_undo_from_result, capture_batch_undo_context};
...
fn execute_cli_batch(cmds: Vec<TicketCommandCli>, store: &TicketStore) -> Result<Value, CliRunError> {
    ...
    for cmd in cmds {
        ...
        match batch_dispatch(cmd, store) {
            Ok(result) => {
                if let Some(undo) = undo_context.and_then(|context| batch_undo_from_result(context, &result, store)) {
                    undo_stack.push(undo);
                }
                results.push(result);
            }
            Err(e) => {
                return Ok(batch_error_response(results.len(), total, e, undo_stack, store));
            }
        }
    }
...
fn rollback_batch(undo_stack: Vec<BatchUndoOp>, store: &TicketStore) -> Vec<String> {
