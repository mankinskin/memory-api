...
fn blocked_transition_reports_current_allowed_and_intermediate_states() {
    ...
    // `new -> in-implementation` is reachable only through `ready`. Under the
    ...
    assert!(
        ...
        "error should list 'ready' as a legal next step: {stderr}"
    );
    assert!(
    ...
    assert!(
        stderr.contains("first transition through") && stderr.contains("ready"),
...
fn transitions_command_shows_legal_transition_graph() {
    ...
    assert_eq!(overview["current_state"], "new");
...
fn legal_single_hop_transition_still_succeeds() {
    ...
    let updated = s.ticket_json(&["update", &id, "--to-state", "ready"]);
...
fn multi_hop_transition_auto_walks_by_default() {
    ...
    // `new -> in-implementation` requires passing through `ready`. Without the
